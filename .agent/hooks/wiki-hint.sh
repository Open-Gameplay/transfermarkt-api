...
case "$b" in
  # Входная точка и сборка роутов
  main.py|api.py|settings.py)
    pages="архитектура" ;;
  # Эндпоинты
  players.py|clubs.py|national_teams.py|coaches.py|competitions.py)
    pages="эндпоинты архитектура" ;;
  # Скрейперы
  base.py|utils.py|xpath.py|regex.py)
    pages="архитектура" ;;
  # Схемы
  profile.py|market_value.py|players.py|search.py)
    pages="архитектура эндпоинты" ;;
  *) ;;
esac
# ──────────────────────────────────────────────────────────────────────────────
